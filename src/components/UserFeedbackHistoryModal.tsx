...
export const UserFeedbackHistoryModal: React.FC<UserFeedbackHistoryModalProps> = ({
  ...
  return (
    ...
    >
      <div 
        className="bg-white rounded-lg max-w-lg w-full p-6 sm:p-7 shadow-2xl border border-slate-200 relative text-[#1a1c2e] animate-scale-up max-h-[85vh] flex flex-col"
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 pb-3 mb-3 border-b border-slate-200">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-md bg-[#6c5ce7] hover:bg-[#5b4bc4] text-white text-xs font-bold flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
          >
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-3">
          {loading ? (
            ...
            feedbackList.map((item) => (
              <div 
                key={item.id} 
                className="p-4 rounded-md bg-slate-50 border border-slate-200 hover:border-slate-300 transition-all space-y-2"
              >
        ...
        <div className="pt-4 mt-2 border-t border-slate-200 flex justify-end">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 rounded-md bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs transition-colors cursor-pointer border border-slate-200"
          >
            Close
          </button>
        </div>
